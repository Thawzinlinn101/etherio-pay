...
const Transfered = (props) => {
    ...
    return (
        <View style={{ flex: 1 }}>
            <Text style={{ color: '#e041b1', margin: 15, fontSize: 20 }}>Transfered Logs</Text>
            <ScrollView style={{ paddingHorizontal: 15 }}>
                <DataTable>
                    <DataTable.Header>
                        <DataTable.Title style={{ justifyContent: 'center', alignContent: 'center' }}>To</DataTable.Title>
                    ...
                    {data.map(row => (
                        <DataTable.Row>
                            <DataTable.Cell style={{ justifyContent: 'center', alignContent: 'center' }}>{(row.recipientName || '').replace(/^\+95/, '0')}</DataTable.Cell>
